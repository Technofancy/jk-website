...
import { fetchPress, fetchPressBySlug } from "../api/press";
...
function PressDetailModal({ item, onClose, loading }) {
      ...
      >
        {loading ? (
          <div className="flex-grow flex items-center justify-center">
            <Loading text={t("pressPage.loadingDetails")} />
          </div>
        ) : (
          item && (
            <>
...
export default function PressReleasePage() {
  ...
  const loadItems = useCallback(
    async (nextPage = 1) => {
      ...
      setLoading(true);
      try {
        const { items: data, totalPages: newTotalPages } = await fetchPress(
          nextPage,
          9
        );
  ...
  const handleCardClick = async (slug) => {
    ...
    try {
      const itemDetails = await fetchPressBySlug(slug);
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto p-4 space-y-8 text-text-default pb-6 min-h-screen">
        ...
        >
          <h1 className="text-5xl font-bold mb-4">{t("pressPage.title")}</h1>
          <p className="text-xl">{t("pressPage.subtitle")}</p>
        </motion.section>
        ...
        >
          <div className="relative">
            ...
            <input
              type="text"
              placeholder={t("pressPage.placeholder")}
              value={searchQuery}
